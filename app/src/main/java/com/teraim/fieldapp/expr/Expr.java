...
class BinaryExpr extends Expr {
 ...
 public Double value() {
	 ...
	 if (rator == OR && (rand0== null || rand1 == null)) {
		/* if (rand0==null&&rand1==null) {
			 System.out.println("Binary expression evaluates to null for OR since both args were null");
			 return null;
		 }
		 */
		 if (rand0==null) {
			 System.out.println("Binary or with rand0 null and rand1: "+rand1);
			 return (rand1.value() != 0.0)?1.0:0.0;
		 }
		 else {
			 System.out.println("Binary or with rand1 null and rand0: "+rand0);
			 return (rand0.value() != 0.0)?1.0:0.0;
		 }
			 
	 }
	Double darg0 = rand0.value();
	Double darg1 = rand1.value();
	if (darg0 == null || darg1 == null) {
		if (rator == OR) {
			if (darg0!=null) {
				System.out.println(darg0+" OR null evaluates to "+(darg0 != 0?1.0:0.0));
				return darg0 != 0?1.0:0.0;
			}
			if (darg1!=null) {
				System.out.println("null OR "+darg1+" evaluates to "+(darg1 != 0?1.0:0.0));
				return darg1 != 0?1.0:0.0;
			}
			else {
				System.out.println("OR with null in both arguments. Returning null");
				return null;
			}
		} else {
			System.out.println(rator+" rator with arguments "+darg0+","+darg1+" .Returning null");
			return null;
		}
	}
	double arg0 = darg0,arg1=darg1;
	switch (rator) {
	...
	case MIN:   return arg0 < arg1 ? arg0 : arg1;
     ...
     case EQ: 
    	 System.out.println(arg0+" = "+arg1+" evaluates to "+(arg0 == arg1));
...
class ConditionalExpr extends Expr {
 ...
 public Double value() {
	 if (test==null)
		 return null;
	 if (test.value()!=0 && consequent == null)
		 return null;
	 if (test.value()==0 && alternative == null)
	 	return null;
